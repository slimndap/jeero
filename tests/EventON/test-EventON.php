...
class EventON_Test extends Jeero_Test {
	
	function test_plugin_activated() {
		
		$actual = class_exists( 'EventON' );
		$expected = true;
	...
	function test_calendar_in_subscription_edit_form() {
		...
		$expected = '<input name="calendar[]" type="checkbox" value="EventON">';
	...
	function test_inbox_event_uses_custom_templates() {
		...
		$settings = array(
			'theater' => 'veezi',
			'calendar' => array( 'EventON' ),
			'EventON/import/template/title' => '{{title}} with custom template',
			'EventON/import/template/content' => '{{description|raw}}{% if tickets_url %}<h3>Tickets</h3>{{tickets_url}}{% endif %}',
		);
	...
	function test_inbox_event_uses_custom_template_fields() {
		...
		$settings = array(
			'theater' => 'veezi',
			'calendar' => array( 'EventON' ),
			'EventON/import/template/content' => '<h3>{{ subtitle }}</h3>{{description|raw}}',
		);
	...
	function test_inbox_event_silently_fails_incorrect_templates() {
		...
		$settings = array(
			'theater' => 'veezi',
			'calendar' => array( 'EventON' ),
			'EventON/import/template/title' => '{% if xxx}{{ title }}',
		);
		...
		$actual = $events[ 0 ]->post_title;
		$expected = 'Rendering template for EventON field failed.';
		$this->assertContains( $expected, $actual );	
	...
	function test_inbox_event_imports_custom_fields() {
		...
		$settings = array(
			'theater' => 'veezi',
			'calendar' => array( 'EventON' ),
			'EventON/import/template/custom_fields' => array(
				array(
